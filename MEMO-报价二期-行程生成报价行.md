...
> 日期：2026-09-09 · **修订：2026-09-10（对齐三层「按日一行」）**

---

## 审阅结论（2026-09-10）

规划**方向正确，可以开工**；下列口径必须按现行三层改，否则会做错模型：

| 原规划写法 | 问题 | 落地口径 |
|------------|------|----------|
| 一日多行（费用类型 hotel/vehicle/…） | 已改为**按日一行**点选产品 | 幂等键：`{route}\|d{day}\|day`；一行内填 酒店/车型A/B/导游 |
| `pricingDays` + 金额覆盖 | 表已是 `pricingDaySheet`，价在产品库 | 写 relation，不写金额列；prune 条件改为「无产品勾选」 |
| 估算回退旧七模块 | 已删除 | 有 days 才估价；无 days 询价 |
| 「连住易重复建行」 | 一日一行后是「漏天」问题，不是同日多行 | align 查 day 差集即可 |
| 每天强制 guide | 国内司兼导日费常为 0，仍可挂 relation | 有住或有车的日才预填导；送机日可不填 |
| train → 独立费用行 | 按日一行无 train 类型 | 酒店空着；align 提示「米轨卧铺待加杂项」 |
| r2/r3 映射不全 | guantang / beihai / weizhou 产品库可能尚未建 | **仍生成空行**；酒店 unmapped 待选，不阻塞建表 |

其余原则保留：不覆盖人工选项、人工锁定、不自动造票餐价、CLI 首版、scaffold→Notion→`content:notion`。
