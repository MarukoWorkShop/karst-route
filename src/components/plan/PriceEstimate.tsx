 ...
 * 金额与路线卡共用 formatMoney + type-price。
 */
export function PriceEstimate({
  ...
  if (!pricingAvailable(route)) {
    return (
      <aside className={`rounded-lg border border-line/80 bg-bone/40 px-4 py-3.5 ${className}`}>
        <p className="type-aux text-ink-soft">{t(copy.plan.estUnavailable)}</p>
      </aside>
    );
  }
  ...
  if (!market && !est) {
    return (
      <aside className={`rounded-lg border border-line/80 bg-bone/40 px-4 py-3.5 ${className}`}>
        <p className="type-aux text-ink-soft">{t(copy.plan.estOversize)}</p>
      </aside>
    );
  }
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-end justify-between gap-2 border-b border-gold/20 px-4 py-3">
        <div>
          <p className="type-meta text-gold">{t(copy.plan.estBadge)}</p>
          <p className="type-sub mt-0.5 font-semibold text-ink">{t(copy.plan.estTitle)}</p>
        </div>
        <div className="flex flex-col items-end gap-1.5">
          {locale === "en" ? <CurrencyToggle value={currency} onChange={setCurrency} /> : null}
          <p className="type-aux text-ink-soft">
      ...
      <div className="px-4 pt-1 pb-2">
        <div className="flex items-baseline justify-between gap-3 border-b border-line/70 py-2.5">
          <span className="type-aux text-ink-soft">{t(copy.plan.estMarketAdult)}</span>
          <span className="type-price text-ink">
            {money(adultPer)}
        ...
        <div className="flex items-baseline justify-between gap-3 border-b border-line/70 py-2.5">
          <span className="type-aux text-ink-soft">{t(copy.plan.estMarketChild)}</span>
          <span className="type-price text-ink">
            {money(childPer)}
            <span className="type-meta ml-1 font-normal normal-case tracking-normal text-ink-soft">
              {t(copy.plan.estPerUnit)}
        ...
        <div className="flex items-baseline justify-between gap-3 py-3">
          <span className="type-aux font-semibold text-ink">{t(copy.plan.estSubtotal)}</span>
          <span className="type-price text-cta">{money(subtotal)}</span>
        </div>
      </div>

      <div className="space-y-2 border-t border-gold/20 bg-bone/35 px-4 py-3.5">
        {locale === "en" ? (
          <p className="type-aux text-ink-soft">{t(copy.plan.estFxNote)}</p>
        ) : null}
        {est?.fleetAdjusted ? (
          <p className="type-aux text-ink-soft">{t(copy.plan.estFleetHint)}</p>
        ) : null}
        {children > 0 ? (
          <p className="type-aux text-ink-soft">{t(copy.plan.estChildrenHint)}</p>
        ) : null}
        <p className="type-aux font-semibold text-ink">{t(copy.plan.estDisclaimer)}</p>
        <p className="type-aux text-ink-soft">{t(copy.plan.estConciergeHint)}</p>
      </div>
    </aside>
  );
}
